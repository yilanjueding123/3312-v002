...
static INT32U R_gc1004_version = C_GC1024;
...
static void gc1024_cvt_analog_gain(INT32U analog_gain)
{
...
static void gc1064_cvt_analog_gain(INT32U analog_gain)
{
...
void gc1004_set_exposure_gain(void)
{
    ...
    else if(R_gc1004_version == C_GC1064)
    {
    ...
    {
        gc1024_cvt_analog_gain(gc1004_seInfo.analog_gain);
    }
    //DBG_PRINT("G");
}
...
void sensor_gc1004_init(INT32U WIDTH, INT32U HEIGHT)
{
    ...
    */
    ...
    //DBG_PRINT("gc1004_check1\r\n");
    gc1004_init();

    //DBG_PRINT("gc1004_check2\r\n");
    ...
    if(sensor_format == GC1004_RAW)
    {
        if(WIDTH == 1280 && HEIGHT == 720)
        {
            ...
            if (R_gc1004_version == C_GC1024)
            {
                for (i = 0; i < sizeof(GC1024_Para_720P_30) / 2; i++)
                {
                    sccb_write(GC1004_SLAVE_ID, GC1024_Para_720P_30[i][0], GC1024_Para_720P_30[i][1]);
                }
            }
            else if(R_gc1004_version == C_GC1064)
            {
    ...
    else if(sensor_format == GC1004_MIPI)
    {
        ...
        else if	(WIDTH == 1280 && HEIGHT == 720)
        {
            ...
            {
                for (i = 0; i < sizeof(GC1024_MIPI_720P) / 2; i++)
                {
                    reg_tmp = GC1024_MIPI_720P[i][0];
                    data_tmp =  GC1024_MIPI_720P[i][1];
                    sccb_write(GC1004_SLAVE_ID, GC1024_MIPI_720P[i][0], GC1024_MIPI_720P[i][1]);
#if 1
                    if (reg_tmp == 0x10 && data_tmp == 0x80)
                    {
