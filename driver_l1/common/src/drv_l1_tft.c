...
void AP_TFT_ClK_144M_set(void)
{
    ...
    tft_clk_set(TFT_CLK_DIVIDE_6);//TFT_CLK_DIVIDE_8
...
#else
    R_TFT_HS_WIDTH			= 1;				//	1		=HPW
    R_TFT_H_START			= 1 + 89; 			//	240 	=HPW+HBP
    R_TFT_H_END 			= 1 + 89 + 1440;		//	1520	=HPW+HBP+HDE
    R_TFT_H_PERIOD			= 1 + 89 + 1440 + 30; //	1560	=HPW+HBP+HDE+HFP
    R_TFT_VS_WIDTH			= 1;				//	1		=VPW				(DCLK)
    R_TFT_V_START			= 21 + 1; 			//	21		=VPW+VBP			(LINE)
    R_TFT_V_END 			= 21 + 1 + 240; 		//	261 	=VPW+VBP+VDE		(LINE)
    R_TFT_V_PERIOD			= 21 + 1 + 240; 	//	262 	=VPW+VBP+VDE+VFP	(LINE)
    R_TFT_LINE_RGB_ORDER	= 0x00;
#endif
