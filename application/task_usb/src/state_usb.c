...
static INT8U usb_state_flag = 0; //0:拸USB 拸鳶籟  1:USB 2:鳶籟
///////////////// CHARGE ////////////////
...
void state_usb_entry(void *para1)
{
    ...
    while (1)
    {
        ...
        case MSG_USBCAM_INITIAL:
            ...
            if(screen_saver_enable)		//if screen is closed, open it, then handle next things.
            {
                screen_saver_enable = 0;
                ap_state_handling_lcd_backlight_switch(1);
            }

            if  ( (usb_status == USB_MSDC) || (usb_status == USB_UVC) )		//u or sd
            {
