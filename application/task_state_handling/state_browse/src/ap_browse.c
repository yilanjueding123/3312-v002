...
void ap_browse_init(INT32U prev_state, INT16U play_index)
{
    ...
    ap_state_handling_icon_show_cmd(ICON_PLAYBACK, NULL, NULL);				//show exit button when enter another app.
    DBG_PRINT("ICON_PLAYBACK_SHOW1\r\n");
...
void ap_browse_string_icon_clear(void)
{
    ...
    DBG_PRINT("ap_browse_string_icon_clear\r\n");
}

void ap_browse_exit(INT32U next_state_msg)
{
    DBG_PRINT("--Brose_exit---\r\n");
    ...
    if((next_state_msg == MSG_APQ_MODE) || (next_state_msg == MSG_APQ_POWER_KEY_ACTIVE))
    {
        ...
        DBG_PRINT("ICON_PLAYBACK_CLEAR1\r\n");
...
//static INT32U file_index_type;
INT32U copy_filename_inf(void)
{
    //	file_index_type=browse_curr_avi.play_index ;
    //DBG_PRINT("brose_file_name=%x\r\n",file_index_type);
    //file_index_type=file_index_type << 8;
    //DBG_PRINT("brose_file_name=%x\r\n",file_index_type);
    //file_index_type=file_index_type|browse_curr_avi.file_type;
    //DBG_PRINT("brose_file_name=%x\r\n",file_index_type);
    return browse_curr_avi.file_path_addr;
...
void ap_browse_reply_action(STOR_SERV_PLAYINFO *info_ptr)
{
    ...
    if (info_ptr->err_flag == STOR_SERV_OPEN_OK)
    {
        ...
        //DBG_PRINT("ICON_PLAYBACK_CLEAR2\r\n");

        gp_memcpy((INT8S *)&browse_curr_avi, (INT8S *)info_ptr, sizeof(STOR_SERV_PLAYINFO));
        //	DBG_PRINT("brose_file_name=%d\r\n",browse_curr_avi.play_index);
        copy_filename_inf();
...
void ap_browse_resolution_string_draw(STOR_SERV_PLAYINFO *info_ptr, INT16U wAVIWidth)
{
    DBG_PRINT("ap_browse_resolution_string_draw\r\n");
    ap_state_handling_ASCII_str_draw_exit(&browse_resolution_str, 1);
...
void ap_browse_show_file_name(INT8U enable, INT32U buf_addr)
{
    ...
    if(enable)
    {
        ...
        else     //HDMI
        {
            ...
            ascii_str.font_type = 0; //big ascii
            ascii_str.pos_x = 600;
            ascii_str.pos_y = 30;
            ascii_str.str_ptr = (CHAR *) browse_curr_avi.file_path_addr;
...
INT32S ap_browse_mjpeg_decode(void)
{
    ...
    if (browse_curr_avi.file_handle >= 0)
    {
        ...
        lseek(browse_curr_avi.file_handle, 0x04, SEEK_SET);
        read(browse_curr_avi.file_handle, (INT32U)&avi_filesize, 4);
        lseek(browse_curr_avi.file_handle, 0x00, SEEK_SET);
        if (vid_dec_parser_start(browse_curr_avi.file_handle, 1, avi_filesize + 8))
        {
            ret = STATUS_FAIL;
        }
        if (ret != STATUS_FAIL)
