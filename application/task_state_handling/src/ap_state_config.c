...
*/
void set_check_flag(INT8U posation, INT8U value)
{
    if(value)
    {
        CHECK_RES |= (1 << posation);
    }
    else
    {
        CHECK_RES &= (~(1 << posation));
...
INT8U get_check_flag(INT8U posation)
{
    if(CHECK_RES & (1 << posation))
        return 1;
    else
        return 0;
}
...
#endif
    ap_state_config_pic_size_set(6);//1280 *960
    ap_state_config_language_set(2);
    //ap_current_state_set(State_video);
    ap_state_config_record_time_set(3);
    ap_state_config_auto_off_set(0);
    ap_state_config_auto_off_TFT_BL_set(1);
    //set_pa_cnt(128);

    Global_User_Optins.item.ifdirty = 1;
}

void ap_state_config_store(void)			//auto save, write infomation to spi,when startup agin, can read prevous state.
{
    return;
