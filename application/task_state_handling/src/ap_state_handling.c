...
#endif
#if 1
static INT32S charge_ptr;
INT32U charge_display_buf;
#endif
...
void ap_state_handling_ASCII_str_draw_exit(STR_ICON_EXT *str_ptr, INT8U wait)
{
    ...
    //DBG_PRINT("ap_state_handling_ASCII_str_draw_exit=%s",str_ptr);
    if (str_ptr->addr)
    {
...
void ap_state_handling_connect_to_pc(INT32U prev_state)
{
    ...
    //if(ap_state_handling_storage_id_get() == NO_STORAGE)//无卡进PCCAMMER有卡进USB
    //	temp = 1;
    //else
    //	temp=0;
...
extern INT8U brose_flag;
void ap_pa_ctrl(void)
{
    if(pa_cnt == 0)
...
#define NOP   {INT16U i=1000;while(i--);}
static INT8U dete_batt_state(void)
{
    INT8U result = NULL;
    INT16U pin_state = 0;
    static INT8U charge_full_cnt = 0;
    static INT8U charge_null_cnt = 0;
    pin_state = gpio_read_io(BATT_CHARGE);
    if(pin_state)
    {
        charge_full_cnt++;
        charge_null_cnt = 0;
...
static void display_charge(INT8U step)
{
    INT16U charge_fd;
    INT32S  status;
    INT32U  size;
    static INT8U prev_state = 0x55;


    if(prev_state == step)
        return ;
    DBG_PRINT("dispaly_charge\r\n");
    if(step)
        charge_fd = nv_open((INT8U *) "CHARGE.JPG");
    else
        charge_fd = nv_open((INT8U *) "CHARGE_FULL.JPG");
    if (charge_fd != 0xFFFF)
    {
        do
        {

            charge_display_buf = ap_display_queue_get(display_isr_queue);
            OSTimeDly(10);
        }
        while(!charge_display_buf);

        size = nv_rs_size_get(charge_fd);
        charge_ptr = (INT32S) gp_malloc(size);
        if(charge_ptr)
        {
            if (nv_read(charge_fd, (INT32U) charge_ptr, size) == 0)
            {

                IMAGE_DECODE_STRUCT img_info;

                img_info.image_source = (INT32S) charge_ptr;
                ...
                img_info.output_buffer_pointer = charge_display_buf;
                status = jpeg_buffer_decode_and_scale(&img_info);
            }

            if(status != STATUS_FAIL)
            {
                OSQPost(DisplayTaskQ, (void *) (charge_display_buf | MSG_DISPLAY_TASK_MJPEG_DRAW));
                OSTimeDly(100);
            }
            gp_free((void *) charge_ptr);
            prev_state = step;
        }
    }
...
void ap_state_handling_power_off(INT32U wakeup_flag)
{
    ...
    INT8U current_bat_state;
    DBG_PRINT("---power_off_display---\r\n");
    set_pa_cnt(128);
    ...
    if(!wakeup_flag)
    {
        ...
        DBG_PRINT("POWER_LOGO_READ_FAIL\r\n");
        if (logo_fd != 0xFFFF)
        {
            do
            {
...
#endif
    ////0515
    //gpio_write_io(SPEAKER_EN, 0); //turn off local speaker before dac_disable();
    //ap_pa_ctrl(0);
    set_pa_cnt(0);
    gpio_write_io(SPEAKER_EN, 0);
    ///////////////////////////////////////////////这里关于USB连接关机
    if(sys_pwr_key1_read())
    {
        ...
        while(1)
        {
            ...
            if(usb_cnt > 10)
                break;
            current_bat_state = dete_batt_state();
            if(current_bat_state)
            {
                if(current_bat_state == 1)
                    display_charge(0);
                else if(current_bat_state == 2)
                    display_charge(1);
            }
            OSTimeDly(1);
        }
    ...
    //DBG_PRINT("yyyyyyyyyyyyyyyyyyyy\r\n");
...
#if 0
    system_set_pll(6);
    sys_ir_delay(234); /* wait 10ms to discharge system power */

    R_SYSTEM_CLK_EN0 = /* 0x0015 */0x0017;	//wwj modify, don't turn off memory clock
    R_SYSTEM_CLK_EN1 = 0xC080;
#endif
    //system_set_pll(6);
    //sys_ir_delay(300);	/* wait 10ms to discharge system power */
    ...
    //	DBG_PRINT("ffffffffffffffffffff\r\n");
...
INT32S scaler_set_parameters(void)
{
    FP32   N;
    INT32U factor;
    DBG_PRINT("------RUN_scaler_set_parameters-----\r\n");
    jpeg_output_h_cnt = 0;
    jpeg_scale_h_size = 0;
    jpeg_scale_h_fifo = 0;
...
INT32S jpeg_buffer_decode_and_scale(IMAGE_DECODE_STRUCT *img_decode_struct)
{
    ...
    //DBG_PRINT("-----thumbnail_exist=%d-----\r\n",thumbnail_exist);

    //DBG_PRINT("-----thumbnail_width=%d-----\r\n",thumbnail_width);

    //DBG_PRINT("-----thumbnail_height=%d-----\r\n",thumbnail_height);

    //DBG_PRINT("-----thumbnail_yuv_mode=%d-----\r\n",thumbnail_yuv_mode);
