...
INT8S ap_video_record_init(INT32U state)
{
    ...
    {
        date_time_force_display(1, DISPLAY_DATE_TIME_RECORD);//时钟时间
    }
    else
    {
    ...
    */
    ...
    {
        DBG_PRINT("XXXXXXXXXXX\r\n");
        ap_state_handling_icon_show_cmd(ICON_VIDEO_RECORD, ICON_SD_CARD, NULL);
        ap_video_record_sts_set(~VIDEO_RECORD_UNMOUNT);
        video_calculate_left_recording_time_enable();//卡录像剩余时间
#if C_MOTION_DETECTION == CUSTOM_ON
        if (ap_state_config_md_get())
        {

            DBG_PRINT("YYYYYYYYYYYY\r\n");
...
INT8S ap_video_record_func_key_active(INT32U event)
{
    ...
    if (video_record_sts & VIDEO_RECORD_BUSY)
    {
        //OSTimeDly(500);
        video_down_flag = 1;
        if((event == MSG_APQ_FUNCTION_KEY_ACTIVE) || (event == MSG_APQ_MODE) || (event == MSG_APQ_MENU_KEY_ACTIVE) || (event == MSG_APQ_CAPTUER_ACTIVE)/* || (event == MSG_APQ_POWER_KEY_ACTIVE)*/)
        {
    ...
    {
        if ((video_record_sts & VIDEO_RECORD_UNMOUNT) == 0)			//有卡
        {
#if C_MOTION_DETECTION == CUSTOM_ON
