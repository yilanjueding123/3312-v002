...
#if 1 // (Enable_Lane_Departure_Warning_System)
...
#if 0 //(Enable_Lane_Departure_Warning_System)
...
void ap_get_ldw_display_coordinate(void)
{
	...
	ldw_dis.ldw_dis_LT_alarmP_Y = divFun((LDWPar.LT_alarmP_Y * LDWPar.scaleRate_F),
		 10) +LDWPar.ROI.y + LDWPar.ROI_initY;		// + 10;
	ldw_dis.ldw_dis_LB_alarmP_X = divFun((LDWPar.LB_alarmP_X * LDWPar.scaleRate_F), 10) +LDWPar.ROI.x;
	ldw_dis.ldw_dis_LB_alarmP_Y = divFun((LDWPar.LB_alarmP_Y * LDWPar.scaleRate_F),
		 10) +LDWPar.ROI.y + LDWPar.ROI_initY;		//+ 10;
	...
	ldw_dis.ldw_dis_RT_alarmP_Y = divFun((LDWPar.RT_alarmP_Y * LDWPar.scaleRate_F),
		 10) +LDWPar.ROI.y + LDWPar.ROI_initY;		//+ 10;
	ldw_dis.ldw_dis_RB_alarmP_X = divFun((LDWPar.RB_alarmP_X * LDWPar.scaleRate_F), 10) +LDWPar.ROI.x;
	ldw_dis.ldw_dis_RB_alarmP_Y = divFun((LDWPar.RB_alarmP_Y * LDWPar.scaleRate_F),
		 10) +LDWPar.ROI.y + LDWPar.ROI_initY;		//+ 10;
...
void state_video_record_entry(void * para, INT32U state)
{
	...
	para == STATE_STARTUP) // 開機後進來
	{
		...
		DBG_PRINT("RUN0\r\n");
		 ...
		 (* (INT32U *) para == STATE_VIDEO_PREVIEW)) // 菜單後進來
	{
	...
	//	ap_current_state_set(State_video);
	DBG_PRINT("RUN2\r\n");
	...
	while (exit_flag == EXIT_RESUME)
	{
		...
		switch (msg_id)
		{
			...
			case MSG_APQ_QUICK_RECORD:
				if (! (ap_video_record_sts_get() &VIDEO_RECORD_BUSY)) //not busy states
				{
			...
			case MSG_APQ_VIDEO_RECORD_ACTIVE:
				...
				{
					if (video_record_sts & VIDEO_RECORD_BUSY) // if recording, stop it
						ap_video_record_func_key_active(msg_id);
				}
			...
			case MSG_APQ_MENU_KEY_ACTIVE:
				if ((ap_video_record_sts_get() &VIDEO_RECORD_BUSY)) //recording status, done nothing.
				{
					break;
				...
				ap_state_handling_str_draw_exit(); //display exit string, then enter into setting status.
			...
			case MSG_APQ_MODE:
				...
				if ((ap_video_record_sts_get() &VIDEO_RECORD_BUSY))
				{
					//if(ap_video_record_func_key_active(msg_id)== 0) {
					//	video_key_flag = 0;
					//	break;
					//}
					break;
