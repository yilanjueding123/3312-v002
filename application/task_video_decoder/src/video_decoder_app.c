...
INT32S video_decode_jpeg_as_rgb565_by_piece(INT16S fd, INT32U file_size, INT32U output_width, INT32U output_height, INT32U output_addr)
{
    ...
    for(i = 0; i < 256; i++)
    {
        if(i % 16 == 0)
            DBG_PRINT("\r\n");
        DBG_PRINT("%x ", *data_addr);
        data_addr++;

    }
    data_addr = data_addr - 256;
#endif
    //tempdada= (INT8U *)output_addr;
#if 0
    for(i = 0; i < 128; i++)
    {
        tempdada = (*(volatile INT8U *)file_buff_addr );
        file_buff_addr++;

        DBG_PRINT("%x ", tempdada);
        //output_addr++;
    }
    file_buff_addr = file_buff_addr - 128;
#endif
    // Parse JPEG data header
    jpeg_decode_init();
    ...
    img_src_valid_width = img_valid_width ;
    img_src_valid_height = img_valid_height ;
    //	DBG_PRINT("---img_valid_width=%d---\r\n",img_valid_width);

    //	DBG_PRINT("---img_valid_height=%d---\r\n",img_valid_height);

    //	DBG_PRINT("---img_extend_width=%d---\r\n",img_extend_width);

    //	DBG_PRINT("---img_extend_height=%d---\r\n",img_extend_height);

    // Setup JPEG and Scaler engine for decoding
    scaler_init(SCALER_1);
    //scaler_fifo_line_set(SCALER_1, C_SCALER_CTRL_IN_FIFO_DISABLE);		// Default disable
    scaler_input_format_set(SCALER_1, C_SCALER_CTRL_IN_YUYV);//   C_SCALER_CTRL_IN_YUYV
    scaler_output_format_set(SCALER_1, C_SCALER_CTRL_OUT_RGB565);
    scaler_out_of_boundary_mode_set(SCALER_1, 1);
    scaler_out_of_boundary_color_set(SCALER_1, 0x008080);			// Black
