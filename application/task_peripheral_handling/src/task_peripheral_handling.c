    ...
    while(1)
    {
        ...
        case MSG_PERIPHERAL_TASK_AD_DETECT_CHECK:
            ...
            if(usb_detect_start == 1)
            {
                ...
                if(sensor_error_power_off_timer > 3 * 128 / PERI_TIME_INTERVAL_AD_DETECT) //5s
                {
                    sensor_error_power_off_timer = 0;

                    if(cdsp_overflow_count > 2 * 3) //over 3 times per second
                    {
                        DBG_PRINT("cdsp overflow %d, Auto Power Off...\r\n", cdsp_overflow_count);
                        PreviewIsStopped = 1;

                        drvl1_cdsp_stop();
                        video_preview_stop(1);    // stop sensor
                        gStop_Sensor = 1;
                    }
                ...
                {
                    if(scaler_stopped_timer > 3 * 128 / PERI_TIME_INTERVAL_AD_DETECT)
                    {
